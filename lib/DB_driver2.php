...
class DB_driver
{
    ...
    function connect()
    {
        ...
        if (!$this->__conn){
            // Kết nối
            $this->__conn = mysqli_connect('localhost', 'root', '', 'vnexpress.net') or die ('Lỗi kết nối');
    ...
    // Hàm lấy danh sách theo dieu kien limit
    function get_list_limit($sql,$start=0,$limit=1)
    {
        // Kết nối
        $this->connect();
        $sql .= $sql." ".$start.",".$limit;
